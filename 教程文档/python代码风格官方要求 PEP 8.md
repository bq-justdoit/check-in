写在最前面，PEP 8 作为python的标准编码要求，值得学习。
当前Pycharm工具支持代码自动格式化(快捷键CTRL+ALT+L)，因此主要在变量、函数等命名做好规范即可覆盖极大部分的要求。
变量、函数名要求： 简洁明了，以下划线分隔。
如:
变量
	名称 name
	班级名称 calss_name
...
函数：
	加法函数  def add()
	傅里叶变换函数 def fourier_transform()
	
类名（首字母大写，驼峰命名）：
	人类 class Person()
	人类名称 class PersonName()
